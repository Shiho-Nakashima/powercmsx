 ...
 * @copyright  2022 Alfasado Inc. All Rights Reserved.
 */
if (! defined( 'DS' ) ) {
 ...
 */
    ...
    public    $use_cache_id  = false;
    public    $cache_id      = null;
    public    $properties    = [];
    public    $core_tags = [];
    public    $template_file = null;
    public    $in_build      = false;
    public    $nocache       = false;
    public    $in_nocache    = false;
    public    $allow_php     = false;
    public    $quoted_vars   = [];
    public    $functions     = [];
    public    $restore_vars  = [];
    public    $re_compile    = false;
    public    $compile_path  = null;
    public    $compile_key   = null;
    public    $cache_includes= [];
 ...
 */
    public    $tags = [
      'block'       => ['block', 'loop', 'foreach', 'for','section', 'literal', 'queries',
                        'sethashvar', 'dynamicmtml', 'localvars'],
      ...
      'conditional' => ['else', 'elseif', 'if', 'unless', 'ifgetvar', 'elseifgetvar', 'ifinarray',
                        'isarray', 'isset', 'ifextensionloaded'],
      'modifier'    => ['escape' ,'setvar', 'assign', 'format_ts', 'zero_pad', 'trim_to', 'eval',
                        ...
                        'default', 'normalize', 'preg_quote', 'remove_blank', 'increment', 'decrement',
                        'basename', 'cast_to', 'sanitize', 'count_characters', 'set', 'let',
                        'date_modify', 'encode_url_basename', 'decode_url_basename', 'array_unique',
                        'nowrap', 'url2link', 'masking'],
      'function'    => ['getvar', 'trans', 'setvar', 'ldelim', 'include', 'math', 'break', 'rdelim', 
                        ...
                        'triggererror', 'constant', 'fileput', 'unlink', 'set', 'let', 'require'],
 ...
 */
    public    $modifier_funcs = [
      ...
      'title_case' => 'ucwords', 'urldecode' => 'urldecode',
 ...
 */
    public function __store () {
        $this->stored['vars'] = $this->vars;
        $this->stored['local_vars'] = $this->local_vars;
        $this->stored['__stash'] = $this->__stash;
        $this->stored['params'] = $this->params;
        $this->stored['local_params'] = $this->local_params;
        $this->stored['literal_vars'] = $this->literal_vars;
    }
...
/**
 * Generate clone that have no effect on the original.
 */
 ...
 */
    function build_page ( $path, $params = [], $cache_id = '', $disp = false, $src = '' ) {
        ...
        if (!$force_compile && $compile_key ) { // Compile cache.
        ...
        if (! $this->binmode && ( $this->unify_breaks || $this->trim_tmpl ) ) {
 ...
 */
    function build ( $src, $compiled = false, $cache_id = null, $force = false ) {
        ...
        if ( $this->use_cache_id ) $this->cache_id = $cache_id;
        ...
        $this->cache_id = null;
        $build = $build ? preg_replace( "/\n$/s", '', $build ) : '';
        if ( $build && ! $this->binmode && ( $this->unify_breaks || $this->trim_tmpl ) ) {
            $build = $this->finalize( $build );
        }
        $in_include = false;
        if ( isset( $this->__stash['in_include'] ) ) {
            $in_include = $this->__stash['in_include'];
            unset( $this->__stash['in_include'] );
        }
        if (! $in_include ) {
 ...
 */
    function get_template_path ( $path, $continue = true ) {
        ...
        $extension = isset( pathinfo( $path )['extension'] )
                   ? strtolower( pathinfo( $path )['extension'] ) : '';
 ...
 */
    function setup_args ( $args, $name = '', $ctx = null, $vars = null ) {
        ...
        $encl = $ctx->csv_enclosure;
        $delim = $ctx->csv_delimiter;
        foreach ( $args as $k => $v ) {
            if (! is_string( $v ) ) {
                // do nothing
            } elseif ( strpos( $v, '$' ) === 0 && $v !== '$' ) { // Variable
                ...
                $args[ $k ] = $arr;
                if ( $name && strpos( $name, 'regex' ) !== false ) continue;
                $array = [];
                foreach ( $arr as $var )
                    $array[] = strpos( $var ,'$' ) === 0
 ...
 */
    function get_any ( $name, $var = null, $recursive = false ) {
        ...
        if ( strpos( $name, '.' ) !== false ) {
            ...
            if ( $var == 'local_vars' ) {
                if ( isset( $this->local_vars[ $name ] ) ) {
                    $v = $this->local_vars[ $name ];
                }
            } else if ( isset( $this->local_vars[ $name ] ) || isset( $this->vars[ $name ] ) ) {
                $v = $this->local_vars[ $name ] ?? $this->vars[ $name ];
            }
            if ( $v !== null ) {
                return $v;
            }
            if ( mb_substr_count( $name, '.' ) == 1 ) {
                ...
                foreach ( $names as $idx => $key ) {
                    ...
                    $linking .= '.' . $key;
                    if ( $var == 'local_vars' ) {
                        if ( isset( $this->local_vars[ $linking ] ) ) {
                            $linking_match[] = $idx;
                        } else if (! $recursive && $this->keys_lower ) {
                            $linking_lower = strtolower( $linking );
                            if ( isset( $this->local_vars[ $linking_lower ] ) ) {
                                $linking_match[] = $idx;
                            }
                        }
                    } else {
                        if ( isset( $this->local_vars[ $linking ] ) || isset( $this->vars[ $linking ] ) ) {
                            $linking_match[] = $idx;
                        } else if (! $recursive && $this->keys_lower ) {
                            $linking_lower = strtolower( $linking );
                            if ( isset( $this->local_vars[ $linking_lower ] ) || isset( $this->vars[ $linking_lower ] ) ) {
                                $linking_match[] = $idx;
                            }
                        }
                    }
                ...
                if ( empty( $linking_match ) ) {
                    ...
                    $linking_match = array_reverse( $linking_match );
                    foreach ( $linking_match as $match ) {
                        $arr = ["['$v_name']"];
                        $linking = $v_name;
                        foreach ( $names as $idx => $key ) {
                            $linking .= '.' . $key;
                            if ( $idx === $match ) {
                                $linking = addslashes( $linking );
                                $arr = ["['$linking']"];
                            } else {
                                $key = addslashes( $key );
                                $arr[] = "['$key']";
        ...
        if ( isset( $v ) && is_array( $v ) && isset( $v['__eval__'] ) ) {
            ...
            $this->force_compile = true;
            if (! $this->in_build ) {
                $v = $this->build( $v['__eval__'], false, md5( $v['__eval__'] ) );
            } else {
 ...
 */
    ...
    function block_loop ( $args, &$content, $ctx, &$repeat, $counter, $id ) {
        if (!$counter ) {
            ...
            if ( count( $params ) === 1 && isset( $params['__array__'] ) ) {
    ...
    function block_for ( $args, $content, $ctx, &$repeat, $counter, $id ) {
        ...
        $vars = isset( $params[ $counter ] ) ? $params[ $counter ]
              : array_slice( $params, $counter, 1, true );
        if ( $vars ||
           ( isset( $ctx->local_vars['loop_type'] ) && $ctx->local_vars['loop_type'] == 1
             && ( isset( $args['from'] ) || isset( $args['to'] ) )
             && $vars === 0 ) ) {
            $repeat = true;
    ...
    function block_localvars ( $args, &$content, $ctx, &$repeat, $counter ) {
        $localvars = ['_localvars_old_token'];
        $scope = $args['scope'] ?? null;
        if ( $scope ) {
            if ( $scope !== 'global' && $scope !== 'local' ) {
                $scope = null;
            } else {
                $scope = $scope === 'global' ? 'vars' : 'local_vars';
            }
        }
        if (! isset( $content ) ) {
            $ctx->localize( $localvars );
            $token = md5( uniqid( '', 1 ) . rand() );
            $ctx->stash( '_localvars_old_token', $token );
            if ( $scope ) {
                $ctx->stash( $token, $ctx->$scope );
                $ctx->$scope = [];
            } else {
                $ctx->stash( $token, [ $ctx->vars, $ctx->local_vars ] );
                $ctx->local_vars = [];
                $ctx->vars = [];
            }
            $repeat = true;
            ...
            $repeat = $ctx->false();
            $old_vars = $ctx->stash( $ctx->stash( '_localvars_old_token' ) );
            if ( $scope ) {
                $ctx->$scope = $old_vars;
            } else {
                list( $ctx->vars, $ctx->local_vars ) = $old_vars;
            }
    ...
    function conditional_if ( $args, $content, $ctx, $repeat, $context = true ) {
          ...
          ,'===','!=','!==','<','>','>=','<=','and','or','true','false','{','}','&&','||', '%', '+', '-', '*'];
            ...
            if ( ( isset( $args['tag'] ) ) && ( $tag = $args['tag'] ) ) {
                ...
                if ( isset( $functions[ $tag ] ) ) {
                    ...
                    if ( is_object( $obj ) && method_exists( $obj, $meth ) ) {
                        ...
                        unset( $tag_args['eq'], $tag_args['not'], $tag_args['ne'],
                               ...
                               $tag_args['tag'], $tag_args['vc'], $tag_args['or'] );
                ...
                if ( isset( $args['op'] ) ) {
                    ...
                    $v = static::math_operation( $args['op'], $v, $args['value'], $args );
                } else {
                    $v = $args['value'];
                    unset( $args['value'] );
                }
                ...
                if ( is_array( $args['name'] ) ) {
                    ...
                    if (! $v && $key === '__array__' && !empty( $value ) ) {
                        if ( is_array( $value ) ) {
                            if ( isset( $args['<name>'] ) ) {
                                $name = $args['<name>'];
                                if ( strpos( $name, '.' ) !== false ) {
                ...
                if ( isset( $args['op'] ) ) {
                    $v = static::math_operation( $args['op'], $v, null, $args );
                }
            }
            unset( $args['name'], $args['this_tag'], $args['note'] );
        }
        if ( ( isset( $v ) && $v ) || !empty( $args ) ) {
            ...
            elseif( isset( $args['vc'] ) ) return version_compare( $v, $args['vc'] ) === 1 ? $true : $false;
            ...
            } elseif( isset( $args['or'] ) ) {
                $vars = is_array( $args['or'] ) ? $args['or'] : explode( '|', $args['or'] );
                if ( in_array( $v, $vars ) ) {
                    return $true;
                } else {
                    return $false;
    ...
    function conditional_ifextensionloaded ( $args, $content, $ctx, $repeat, $counter ) {
        $extension = isset( $args['name'] ) ? $args['name'] : '';
        if (! $extension ) $extension = isset( $args['extension'] ) ? $args['extension'] : '';
    ...
    function function_var ( $args, $ctx ) {
        ...
        if ( isset( $args['op'] ) ) {
            $var = static::math_operation( $args['op'], $var, null );
        }
        return $var;
    }
    ...
    function function_unset ( $args, $ctx ) {
        ...
        if ( is_array( $name ) && isset( $args['<name>'] ) ) {
            ...
            if ( isset( $v_name ) ) {
                ...
                $code1 = $parent
                       ? "if (isset( \$ctx->local_vars{$parent})&&is_array(\$ctx->local_vars{$parent}))unset( \$ctx->local_vars{$arr} );"
                       : "unset( \$ctx->local_vars{$arr} );";
                $code2 = $parent
                       ? "if (isset( \$ctx->vars{$parent})&&is_array(\$ctx->vars{$parent}))unset( \$ctx->vars{$arr} );"
                       : "unset( \$ctx->local_vars{$arr} );";
                if (! isset( $args['scope'] ) ) {
                    eval( $code1 );
                    eval( $code2 );
        ...
        if ( strpos( $name, '[' ) !== false && strpos( $name, ']' ) !== false ) {
            if ( preg_match( '/(^.*)\[(.*?)\]$/', $name, $matchs ) ) {
                ...
                if ( strpos( $matchs[2], '$' ) === 0 ) {
                    $v_name = ltrim( $matchs[2], '$' );
                    $var = $this->get_any( $v_name );
                }
                if (! isset( $args['scope'] ) ) {
                    unset( $ctx->vars[ $matchs[1] ][ $var ], $ctx->local_vars[ $matchs[1] ][ $var ] );
                } else if ( $args['scope'] == 'local' ) {
                    unset( $ctx->local_vars[ $matchs[1] ][ $var ] );
                } else {
                    unset( $ctx->vars[ $matchs[1] ][ $var ] );
                }
                return '';
            }
        }
    ...
    function function_date ( $args, $ctx ) {
        ...
        $date = date( 'YmdHis', $t );
        if ( isset( $args['date_modify'] ) && isset( $args['format'] ) ) {
            $date_modify = false;
            foreach ( $args as $key => $arg ) {
                if ( $key == 'format' ) {
                    break;
                } else if ( $key == 'date_modify' ) {
                    $date_modify = true;
                }
            }
            if ( $date_modify ) {
                $date = $this->modifier_date_modify( $date, $args['date_modify'], $ctx );
            }
        }
    ...
    function function_require ( $args, $ctx ) {
        $f = isset( $args['file'] ) ? $args['file'] : '';
        if (! $f ) return '';
        if (! $f = $ctx->get_template_path( $f ) ) return '';
        ob_start();
    ...
    function modifier_setvar ( $str, $arg, $ctx ) {
        if ( $str === null && !$this->var_nullable ) $str = '';
        $ctx->vars[ $arg ] = $str;
        return '';
    }
    ...
    function modifier_date_modify ( $ts, $arg, $ctx ) {
        if ( $ts === null ) $ts = '';
        $ts = preg_replace( '/[^0-9]/', '', $ts );
        if ( preg_match( "/^[0-9]{12}$/", $ts ) ) $ts .= '00';
        if (! preg_match( "/^[0-9]{14}$/", $ts ) ) {
            return $ts;
        }
        list( $y, $m, $d, $t ) = [ (int)substr( $ts, 0,4 ), (int)substr( $ts, 4,2 ),
                                   (int)substr( $ts, 6,2 ), substr( $ts, 8,6 ) ];
        if (! checkdate( $m, $d, $y ) ) {
            return $ts;
        }
    ...
    function modifier_format_ts ( $date, $format, $ctx, $args = [] ) {
        ...
        if ( $Japanese && ( stripos( $format, 'k' ) !== false || strpos( $format, 'R' ) !== false ) ) {
            // K=令和5年, k=令和05年, R=R5年
            $eras = [
                ['year' => 2018, 'name' => '令和'],
                ['year' => 1988, 'name' => '平成'],
                ['year' => 1925, 'name' => '昭和'],
            ...
            foreach( $eras as $era ) {
                ...
                if ( $year > $base_year ) {
                    ...
                    if ( strpos( $format, 'k' ) !== false ) {
                        if ( $era_year !== '元' ) {
                            $era_year = str_pad( $era_year, 2, 0, STR_PAD_LEFT );
                        }
                        $format = preg_replace( '/k/', $era_name . $era_year, $format, 1 );
                    } else if ( strpos( $format, 'K' ) !== false ) {
                        $format = preg_replace( '/K/', $era_name . $era_year, $format, 1 );
                    } else {
                        $shorten = [
                            '令和' => 'R',
                        ...
                        $era_name = $shorten[ $era_name ];
                        $format = preg_replace( '/R/', $era_name . $era_year, $format, 1 );
                    }
                    break;
                }
            }
    ...
    function modifier_zero_pad ( $str, $arg ) {
        if ( $str === null ) $str = '';
        return str_pad( $str, $arg, '0', STR_PAD_LEFT );
    }

    function modifier_strip_linefeeds ( $str, $arg ) {
        return $str === null ? '' : str_replace( ["\r\n", "\n", "\r"], '', $str );
    }
    ...
    function modifier_encode_url_basename ( $str, $arg ) {
        if ( $str === null ) return '';
        if ( $arg === 'all' ) {
            $base = '';
            if ( preg_match( '!(^https{0,1}://[^/]*?/)(.*$)!', $str, $matchs ) ) {
                $base = $matchs[1];
                $paths = explode( '/', $matchs[2] );
            } else {
                $paths = explode( '/', $str );
            }
            foreach ( $paths as $idx => $path ) {
                $paths[ $idx ] = rawurlencode( $path );
            }
            $paths = implode( '/', $paths );
            return $base . $paths;
        }
        ...
        if ( $basename !== $basename_enc ) {
            $quoted = preg_quote( $basename, '/' );
            $str = preg_replace( "/{$quoted}$/", $basename_enc, $str );
        }
        if ( $arg === 'dirname' ) {
            $str .= "/{$filename}";
        }
    ...
    function modifier_decode_url_basename ( $str, $arg ) {
        if ( $str === null ) return '';
        if ( $arg === 'all' ) {
            $base = '';
            if ( preg_match( '!(^https{0,1}://[^/]*?/)(.*$)!', $str, $matchs ) ) {
        ...
        $basename = basename( $str );
        $basename_decode = urldecode( $basename );
        if ( $basename !== $basename_decode ) {
            $quoted = preg_quote( $basename, '/' );
            $str = preg_replace( "/{$quoted}$/", $basename_decode, $str );
        }
        if ( $arg === 'dirname' ) {
            $str .= "/{$filename}";
        }
    ...
    function modifier_encode_backslash ( $str, $arg ) {
        if ( $str === null ) return '';
    ...
    function modifier_format_size ( $size, $precision, $ctx ) {
        ...
        if ( $size >= 1073741824 ) {
            $size = round( $size / 1073741824, $precision );
            $unit = 'GB';
    ...
    function modifier_regex_replace ( $str, $args, $ctx ) {
        ...
        if ( $delimiter !== '[' ) {
            $count = $pattern && $delimiter ? mb_substr_count( $pattern, $delimiter ) : 0;
            if ( $count === 1 ) {
                // Delimiter must not be alphanumeric or backslash.
    ...
    function modifier_normalize ( $str, $arg, $ctx ) {
        ...
        if ( function_exists( 'normalizer_normalize' ) ) {
            if ( strpos( $arg, 'N' ) === 0 ) {
                $arg = constant( "Normalizer::{$arg}" );
            } else {
                $arg = constant( 'Normalizer::NFKC' );
            }
    ...
    function modifier_basename ( $str, $arg, $ctx ) {
        if ( $str === null ) return '';
        if ( $arg && $arg != 1 && $arg !== 'no_suffix' ) {
            // Could be a block tag modifier.
            return $str;
        }
        if ( preg_match( '/[\n|\r]/', $str ) ) {
            // Not a path.
    ...
    function modifier_array_unique ( $arr, $arg, $ctx ) {
        if (!is_array( $arr ) ) return [];
        return array_unique( $arr );
    }

    function modifier_op ( $str, $arg, $ctx, $op, $args ) {
        if ( $str === null ) $str = '';
        $args = json_decode( $args, true );
        $str = static::math_operation( $op, $str, $arg, $args );
        return $str;
    }

    function modifier_nowrap ( $str, $arg, $ctx ) {
        if ( $str === null ) return '';
        $replace = $arg == 2 ? '' : ' ';
        return str_replace( ["\r\n", "\r", "\n"], $replace, $str );
    }

    function modifier_url2link ( $str, $arg, $ctx ) {
        if ( $str === null ) return '';
        $regex = "!((?:\G|>)[^<]*?)(https?://[-_.\!~*'()a-zA-Z0-9;/?:\@&=+\$,%#\[\]]+)!";
        if (! preg_match( $regex, $str ) ) {
            return $str;
        }
        $attrs = is_string( $arg ) && $arg !== '1' ? $arg : null;
        if ( $attrs === null && is_array( $arg ) ) {
            $attrs = '';
            foreach ( $arg as $name => $value ) {
                $attrs .= " {$name}=\"{$value}\"";
            }
        } else if ( is_string( $attrs ) && $attrs === '_blank' ) {
            $attrs = ' target="_blank"';
        }
        $str = preg_replace( $regex, '$1<a href="$2"' . $attrs . '>$2</a>', $str );
        return $str;
    }

    function modifier_masking ( $str, $arg, $ctx ) {
        if ( $str === null ) return '';
        $arg = $arg ? $arg : '*';
        return str_repeat( $arg, strlen( $str ) );
    }
 ...
 */
    function parse_literal ( &$content, $kind = null ) {
        ...
        if ( $tagname == 'literal' && strpos( $content, 'dynamicmtml' ) !== false ) {
        ...
        if ( strpos( $content, $tagname ) === false ) return false;
 ...
 */
    function pre_fetch ( $content, &$map =[] ) {
        ...
        foreach ( $this->pre_fetch as $regex => $replace ) {
            if ( is_array( $replace ) ) {
                if ( preg_match_all( $regex, $content, $matchs ) ) {
                    $matchs = $matchs[0];
                    foreach ( $matchs as $match ) {
                        $tag = $match;
                        foreach ( $replace as $key => $value ) {
                            $match = preg_replace( $key, $value, $match );
                        }
                        $content = str_replace( $tag, $match, $content );
                    }
                }
            } else {
 ...
 */
    function compile ( $content, $disp = true, $tags_arr = null, $use_tags = [],
        ...
        $prefix = $this->prefix;
        $regex = '/<\${0,1}' . $prefix . '/i';
        if (!empty( $this->pre_fetch ) && !preg_match( $regex, $content ) ) {
        ...
        list( $literals, $templates ) = (!$this->_include )
            ? [ $this->literal_vars, $this->template_paths ] : [ [], $this->template_paths ];
        ...
        if ( $this->request_cache && isset( $this->compiled[ $orig_key ] ) ) {
          ...
          { // Process in descending order of length of the tag names.
            ...
            if (!$this->_include ) if ( strpos( $content, 'literal' ) !== false
                || strpos( $content, 'dynamicmtml' ) !== false )
            ...
            if (! $this->app_name && $prefix == 'mt' && !preg_match( $regex, $content )
                && empty( $this->replaced_map ) ) {
                // for Prototype.
                $this->out = null;
            ...
            foreach ( $tags_arr as $tag )
          {
            ...
            . $tag_e . '/si', '<$1>', $content );
            if ( strpos( $content, 'setvartemplate' ) !== false )
         ...
         // Double escape HTML entities.
            ...
            $documentElements = $dom->documentElement;
            $using_tags = [];
            $tag_regex = "/^{$prefix}/";
            foreach ( $documentElements->childNodes as $child ) {
                if ( property_exists( $child, 'tagName' ) ) {
                    $using_tags[ preg_replace( $tag_regex, '', $child->tagName ) ] = 1;
                    self::get_children_nodes( $child, $using_tags, $tag_regex );
                }
            }
        ...
        {
            if ( $this->include_realtime &&
                ( in_array( $include, $function_tags ) || in_array( $include, $block_tags ) ) ) continue;
            if (!isset( $using_tags[ $include ] ) ) continue;
            ...
            while ( $i > -1 )
          {
            ...
            if (!empty( $this->pre_fetch ) && !preg_match( $regex, $incl ) ) {
                $incl = $this->pre_fetch( $incl, $this->replaced_map );
            }
            if ( strpos( $incl, 'literal' ) !== false ) $this->parse_literal( $incl );
          ...
          { // Processed recursively if included.
            ...
            $build = $this->compile( str_replace( $insert, '', $out ),
                $disp, $tags_arr, $use_tags, [], $compiled, $nocache, $orig_key );
            if ( isset( $sta_h ) && isset( $end_h ) &&
               ( strpos( $build, $sta_h ) !== false || strpos( $build, $end_h ) !== false ) ) {
                $build = str_replace( [ $sta_h, $end_h ], ['<', '>'], $build );
            }
            return $build;
          }
            $pid = '$' . $this->id . '_';
        ...
        {
            if (!isset( $using_tags[ $block ] ) ) continue;
        ...
        {
            if (!isset( $using_tags[ $conditional ] ) ) continue;
        ...
        {
            if (!isset( $using_tags[ $function ] ) ) continue;
        ...
        if ( preg_match_all( "/{$h_sta}\\s*\\\${0,1}({$pfx}|\\\$):{0,1}\\s"
          ...
          { // Convert inline variables to 'var' tag.
            ...
            $this->force_compile = $force_compile;
            $orig_key = $this->cache_id ?? $orig_key;
...
/**
 * List up all tag names.

 * @param  object $domNode   : Object DOMElement.
 * @param  string $using_tags: Array with tag name as key.
 * @param  string $tag_regex : Regular expression to remove leading prefix.
 */
    static function get_children_nodes ( $domNode, &$using_tags = [], $tag_regex = '/^mt/' ) {
 ...
 */
    function set_cache ( $key, $out, $path = null, $req = '', $nocache = false ) {
        ...
        true ) . ';';
        $out = preg_replace( "/\n$/s", '', $out );
        $req .= ( $nocache ) ? '$this->nocache=true;' : '';
        $out ="<?php {$req}ob_start();?>{$out}<?php \$this->out=ob_get_clean();{$meta}?>";
        if ( is_object( $this->cache_driver ) ) {
            return $this->cache_driver->set( $path, $out );
    ...
    function get_cache ( $key, $ttl = null, $comp = null, $path = null ) {
        ...
        if ( is_object( $this->cache_driver ) ) {
            if ( isset( $this->compiled[ $key ] ) ) {
                $compiled = $this->compiled[ $key ];
            } else {
                $compiled = $this->cache_driver->get( $path );
            }
            if ( $compiled && is_string( $compiled ) ) {
                $out = $this->_eval( $compiled );
                if ( $this->out === null && strpos( $compiled, '<?php $this->out=ob_get_clean();' ) === false ) {
                    $this->out = $out;
                }
            }
    ...
    function add_modifier
      ( $out, $attributes, $modifiers, $modifier_funcs, $func_map, &$requires, $esc ) {
        ...
        foreach ( $attributes as $attr_n => $attr_v ) {
            ...
            if ( $this_tag === 'getvar' && $attr_n === 'op' && isset( $attributes['value'] ) ) {
                $value = $attributes['value'];
                $args = json_encode( $attributes );
                unset( $attributes['value'] );
                $out = "\$this->modifier_op({$out},\$this->setup_args"
                     ."('{$value}','{$attr_n}',\$this),\$this,'{$attr_v}','{$args}')";
                continue;
            }
            ...
            if ( $attr_n === 'escape' && ( strtolower( $attr_v ) === 'html' ||
               ...
               'paml_rawurlencode(' . $out . ')':'paml_htmlspecialchars(' . $out . ',ENT_QUOTES)';
                ...
                $func = $modifier_funcs[ $attr_n ];
                if ( $attr_num > 1 && $attr_v && $attr_v !== '1' ) {
                    if (! defined( $attr_v ) ) {
                        $attr_v = $func === 'strtotime' ? "intval('{$attr_v}')" : "'{$attr_v}'";
                    }
                    $out = "paml_modifier_funcs('{$func}', $out, {$attr_v})";
                    // $out = $modifier_funcs[ $attr_n ] . '(' . $out . ",{$attr_v})";
                } else {
                    $out = "paml_modifier_funcs('{$func}', $out)";
        ...
        if ( $esc && (!isset( $attributes['raw'] ) || !$attributes['raw'] ) )
            $out = "paml_htmlspecialchars({$out},ENT_QUOTES)";
        return $out;
    }
 ...
 */
    function finish ( $out, $disp, $cb, $lits = [], $tmpls = [], $nocache = false ) {
        ...
        if (!$this->in_build ) $this->out = null;
        if ( $disp ) echo $out;
        return $out;
    }
...
/**
 * Passing null to parameter #1 ($string) of type string is deprecated in PHP 8.1
 */
function paml_htmlspecialchars ( $str ) {
    return $str === null || is_array( $str ) ? '' : htmlspecialchars( $str, ENT_QUOTES );
}

function paml_rawurlencode ( $str ) {
    return $str === null || is_array( $str ) ? '' : rawurlencode( $str );
}

function paml_modifier_funcs ( $func, $param1, $param2 = null ) {
    if ( $param1 === null ) $param1 = '';
    return $param2 !== null ? $func( $param1, $param2 ) : $func( $param1 );
}