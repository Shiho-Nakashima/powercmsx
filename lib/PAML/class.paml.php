 ...
 */
    public    $tags = [
      ...
      'modifier'    => ['escape' ,'setvar', 'assign', 'format_ts', 'zero_pad', 'trim_to', 'eval',
                        ...
                        'basename', 'cast_to', 'sanitize', 'count_characters', 'date_modify',
                        'encode_url_basename', 'decode_url_basename', 'array_unique',
      ...
      'function'    => ['getvar', 'trans', 'setvar', 'ldelim', 'include', 'math', 'break', 'rdelim', 
                        ...
                        'triggererror', 'constant', 'fileput', 'fileget', 'unlink', 'require'],
 ...
 */
    function build_page ( $path, $params = [], $cache_id = '', $disp = false, $src = '' ) {
        ...
        $ttl = $this->request_time - $filemtime;
        $old_vars = $this->vars;
        ...
        if (!$force_compile && !$src ) { // Compile cache.
 ...
 */
    ...
    function do_modifier ( $name, $out, $arg, $ctx ) {
        ...
        $res = $func( $args[0], $args[1], $args2, $args3, $args4, $args5 );
        return $res !== null ? $res : '';
    }
 ...
 */
    ...
    function block_setvarblock ( $args, &$content, $ctx, &$repeat, $counter ) {
        ...
        if ( isset( $args[ $name ] ) && $args[ $name ] ) {
            if ( isset( $content ) ) {
                ...
                $ctx->vars[ $args[ $name ] ] = $ctx->append_prepend( $content, $args, $name );
            }
        }
        return '';
    }
    ...
    function function_vardump ( $args, $ctx ) {
        ...
        if ( isset( $args['name'] ) && $args['name'] ) {
            ...
            $vars = isset( $vars[ $args['name'] ] ) ? $vars[ $args['name'] ] : null;
            $filter = true;
        }
        if ( isset( $args['key'] ) && $args['key'] ) {
            if ( isset( $vars[ $args['key'] ] ) ) {
                $vars = $vars[ $args['key'] ];
                $filter = true;
            }
        }
    ...
    function function_setvar ( $args, $ctx ) {
        ...
        if ( isset( $args[ $nm ] ) && $args[ $nm ] ) {
            ...
            if ( isset( $args['value'] ) ) {
                if ( isset( $args['op'] ) && isset( $ctx->vars[ $args[ $nm ] ] ) ) {
                    $args['value'] = static::math_operation( $args['op'], $ctx->vars[ $args[ $nm ] ], $args['value'] );
                }
                $ctx->vars[ $args[ $nm ] ] = $ctx->append_prepend( $args['value'], $args, $nm );
            } else if ( isset( $args['op'] ) && isset( $ctx->vars[ $args[ $nm ] ] ) ) {
                $value = static::math_operation( $args['op'], $ctx->vars[ $args[ $nm ] ], null );
                $ctx->vars[ $args[ $nm ] ] = $ctx->append_prepend( $value, $args, $nm );
            }
        }
        return '';
    }
    ...
    function function_fileget ( $args, $ctx ) {
        if (! $this->allow_fileget ) return;
        $path = isset( $args['path'] ) ? $args['path'] : null;
        if (! $path ) return;
        if (! static::allowed_path( $path ) ) return;
    ...
    static function allowed_path ( $path ) {
        $allowed = true;
    ...
    function modifier_op ( $str, $arg, $ctx, $op ) {
        if ( $str === null ) $str = '';
        $str = static::math_operation( $op, $str, $arg );
        return $str;
    }
 ...
 */
    function set_cache ( $key, $out, $path = null, $req = '', $nocache = false ) {
        ...
        $req .= '$this->literal_vars=' . var_export( $this->literal_vars, true ) . ';';
