...
class ExtractContent {
    ...
    public  $image_urls= [];
    public  $base_url  = '';
    public  $punctuation_weight;
    public  $minimum_area = 1500;
    ...
    public function getTextContent () {
        ...
        $dom = new DomDocument();
        if (! $dom->loadHTML( mb_encode_numericentity( $html_content, MB_ENCODE_NUMERICENTITY_MAP, 'utf-8' ),
            LIBXML_HTML_NOIMPLIED|LIBXML_HTML_NODEFDTD|LIBXML_COMPACT ) ) {
            return $content;
        }
        $elements = $dom->getElementsByTagName( 'img' );
        $minimum_area = $this->minimum_area;
        if ( $elements->length ) {
            for ( $i = 0; $i < $elements->length; $i++ ) {
                ...
                if ( $area > $minimum_area ) {
                    ...
                    if ( $src ) {
                        $src = $this->convert2url( $ele->getAttribute( 'src' ) );
                        if (! $this->image_url ) {
                            $this->image_url = $src;
                        }
                        $this->image_urls[] = $src;
    ...
    public function get_main () {
        ...
        if ( $og_image->length ) {
            ...
            if ( $og_image ) {
                $og_image = $this->convert2url( $og_image );
                $this->image_url = $og_image;
                $this->image_urls[] = $og_image;
            }
        }
        ...
        if ( $content ) {
            ...
            if (! $_dom->loadHTML( mb_encode_numericentity( $html_content, MB_ENCODE_NUMERICENTITY_MAP, 'utf-8' ) ) ) {
                return $content;
            } else {
                ...
                if ( $elements->length ) {
                    ...
                    while ( $i > -1 ) {
                        $ele = $elements->item( $i );
                        $i -= 1;
                        if (! $ele || !is_object( $ele ) ) {
                            continue;
                        }
                        $tag_name = property_exists( $ele, 'tagName' ) ? $ele->tagName : '';
                        // form elements
                        if ( $tag_name == 'input' || $tag_name == 'textarea' || $tag_name == 'select' || $tag_name == 'button' ) {
    ...
    public function analyse () {
        ...
        foreach ($list as $block) {
            ...
            $punctuation_weight = $this->punctuation_weight ? $this->punctuation_weight : $this->options['punctuation_weight'];
            $c = strlen($notLinked) + count($punctuations) * $punctuation_weight * $factor;
            ...
            if ($c1 > $this->threshold) {
                $body .= $block . "\n";
                $score += $c1;
                $continuous = $this->options['continuous_factor'];
                $matchContents[ $contentCounter ] = $block;
            } elseif ($c > $this->threshold) {
                $bodyList[] = [
                    $body,
                    $score,
                ];
        ...
        $body = array_reduce($bodyList, function ($a, $b) {
            if (is_array($a) && is_array($b) && ($a[1] >= $b[1])) {
                return $a;
            } else {
                return $b;
            }
    ...
    public function convert2url ( $target_path, $base = '' ) {
