...
class BatchApproval extends PTPlugin {
        ...
        if ( ( $app->mode == 'view' && $app->param( '_type' ) == 'list' )
            ...
            if ( $workflow->id ) {
                ...
                if ( $app->user()->is_superuser ) {
                    ...
                    $group_name = $app->permission_group( $app->user(), $model, $workflow->workspace );
                } else {
                    $workflows = $app->db->model( 'workflow' )->load( ['model' => $model ] );
                    $current_group = 'creator';
                    foreach ( $workflows as $_workflow ) {
                        $_group_name = $app->permission_group( $app->user(), $model, $_workflow->workspace );
                        if ( $_group_name == 'publisher' ) {
                            $current_group = 'publisher';
                            break;
                        } else {
                            if ( $_group_name == 'reviewer' ) {
                                $current_group = 'reviewer';
                            }
                        }
                    }
                    $group_name = $current_group;
                }
                if ( $group_name == 'publisher' && !$app->param( 'manage_revision' ) ) {
        ...
        if ( $app->param( 'do_action' ) && $app->request_method == 'POST'
            ...
            foreach ( $workspace_ids as $workspace_id ) {
                ...
                foreach ( $action_objects as $obj ) {
                    $old_user = $obj->user;
                    if (! $old_user && $obj->user_id ) {
                        $old_user = $app->db->model( 'user' )->load( (int) $obj->user_id );
                    }
                    if (! $old_user ) {
                        $old_user = $app->db->model( 'user' )->new();
                        $old_user->name( '*Unspecified*' );
                        $old_user->nickname( '*Unspecified*' );
                        $old_user->id = $obj->user_id;
                    }
                        ...
                        foreach ( $workspace_ids as $workspace_id ) {
                            ...
                            if ( isset( $ws_changing_user_map[ $workspace_id ] ) ) {
                                ...
                                foreach ( $ws_changes as $user_id => $user_objs ) {
                                    ...
                                    // $old_group_name = $app->permission_group( $old_user, $model, $app->param( 'workspace_id' ) );
                                    $new_group_name = $app->permission_group( $_new_user, $model , $app->param( 'workspace_id' ) );
                                    if ( $new_group_name === 'publisher'
                                        ...
                                        if ( $old_user->email ) {
                                            ...
                                            PTUtil::send_mail(
                                                $app->user()->email, $subject, $body, $headers );
                                        }
                                    }
            ...
            foreach ( $workspace_ids as $workspace_id ) {
                ...
                $group_name = $app->permission_group( $app->user(), $model, $workspace );
                if ( $group_name == 'reviewer' ) {
                    $max_status = 2;
                }
                if ( $max_status < $workflow_status ) {
                    ...
                    if (! empty( $ws_changing_user_map ) ) {
                        if ( isset ( $ws_changing_user_map[ $workspace_id ] ) ) {
                            ...
                            foreach ( $ws_changes as $user_id => $user_objs ) {
                                ...
                                foreach ( $user_objs as $changed_obj ) {
                                    $old_obj = $old_objects[ $changed_obj->id ];
                                    $new_user = isset( $user_map[ $changed_obj->user_id ] ) ? $user_map[ $changed_obj->user_id ]
                                                                                            : $app->db->model( 'user' )->load( $changed_obj->user_id );
                                    $ctx->vars['to_user'] = $new_user->nickname;
                                    $_params = $this->set_params( $app, $changed_obj, $new_user, $primary );
                                    $this->set_defferent_context( $ctx, $old_obj, $changed_obj,
                                                                  $old_user, $app->user() );
    ...
    function finish_action ( $app, $publish_ids, $counter, $publish_objs, $publish_originals ) {
        $model = $app->param( '_model' );
